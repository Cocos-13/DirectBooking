...
export function LocationSection() {
  ...
  const staticMapUrl = mapsKey
    ? "https://maps.googleapis.com/maps/api/staticmap?" +
      [
        ...
        ...MAP_STYLE.map((s) => `style=${encodeURIComponent(s)}`),
        `key=${mapsKey}`,
      ].join("&")
    : null;
  ...
  return (
    <section id="location" className="mx-auto max-w-3xl px-4 py-16 sm:py-20">
      ...
      <Reveal delay={80}>
        <div className="group relative mt-8 overflow-hidden rounded-2xl border border-sand-200 shadow-elev-2 ring-1 ring-aegean-900/5">
          {staticMapUrl ? (
          ...
          <div className="pointer-events-none absolute left-4 top-4 flex items-center gap-2 rounded-full bg-white/95 py-2 pl-3 pr-4 shadow-elev-1 backdrop-blur">
            <svg
              aria-hidden
              viewBox="0 0 24 24"
            ...
            <span className="text-sm font-semibold text-aegean-900">
      ...
      <Reveal delay={120}>
        <a
          ...
          className="group mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-aegean-600 transition-colors hover:text-aegean-700"
        >
          {t.location.mapCta}
